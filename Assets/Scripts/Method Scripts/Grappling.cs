...
public class Grappling : MonoBehaviour
{
	...
	void Update ()
	{
		...
		RaycastHit hit;
		if (Physics.Raycast (ray, out hit, maxDistance, cullingmask.value)) {
	...
	public void Findspot (Ray ray, RaycastHit hit)
	{
		...
		if (Physics.Raycast (target.transform.position, target.transform.forward, out hit, maxDistance, cullingmask.value)) {
			if (hit.collider.tag == "Floor") {
				isMoving = true;
				loc = hit.point;
				LR.SetPosition (1, loc);
